...
class Donchian_ADX_CHOPStrategy(IStrategy):
    ...
    # Wide initial stoploss to allow custom trailing SL
    stoploss = -0.99
    ...
    use_exit_signal = False  # No indicator-based exits; use trailing SL
    exit_profit_only = False
    ignore_roi_if_entry_signal = False

    # Hyperoptable parameters - Donchian
    donchian_period = IntParameter(
        low=10, high=30, default=20, space="buy", optimize=True, load=True
    )
    ...
    chop_period = IntParameter(
        low=10, high=20, default=14, space="buy", optimize=True, load=True
    )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if whale_signal:
            if whale_signal.signal_type == "BUY":
                ...
                whale_long = qtpylib.crossed_above(
                ...
                whale_short = qtpylib.crossed_below(
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
        No indicator-based exits; handled via custom trailing SL.
        """
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        # Find entry index
        entry_idx = dataframe.index.get_loc(
            dataframe[dataframe["date"] == candle_date].index[0]
        )
        ...
        if entry_idx >= current_idx:
            ...
            if not trade.is_short:
                sl_abs = trade.open_rate - (multiplier * entry_atr)
            else:
                sl_abs = trade.open_rate + (multiplier * entry_atr)
        else:
            ...
            if not trade.is_short:
                ...
                sl_abs = max_high - (multiplier * entry_atr)
            else:
                min_low = slice_df["low"].min()
                sl_abs = min_low + (multiplier * entry_atr)
